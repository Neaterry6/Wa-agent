...
async function startServer() {
  ...
  function getState(userId: number) {
    if (!userStates.has(userId)) {
      userStates.set(userId, { model: 'auto', mode: 'roast', cwd: telegramWorkspacesDir, isTerminal: false, buttonMode: true, voiceAiMode: false, zips: {}, pendingZipEdit: false, pendingCreateAppApproval: false });
    }
    return userStates.get(userId)!;
  }

  async function createAppFiles(ctx: Context) {
    const chatId = ctx.chat!.id;
    const state = getState(ctx.from!.id);
    const workspaceDir = path.join(telegramWorkspacesDir, `${ctx.from!.id}_app_${Date.now()}`);
    fs.mkdirSync(workspaceDir, { recursive: true });
    const files: Record<string, string> = {
      "src/index.js": `console.log("Hello World");`,
      "package.json": JSON.stringify({ name: "my-app", version: "1.0.0", scripts: { start: "node src/index.js" } }, null, 2),
    ...
    for (let i = 0; i < entries.length; i += 1) {
      ...
      await bot.telegram.sendMessage(chatId, `📄 Step ${i + 1}/${entries.length}: Writing ${filePath}`);
    }
    state.cwd = workspaceDir;
    await bot.telegram.sendMessage(chatId, `✅ App creation complete!\nWorkspace: ${path.basename(workspaceDir)}`);
  }
...
• /adminadd <id> • /adminremove <id> • /adminlist`;
  ...
  async function handleNaturalChat(ctx: Context, text: string) {
    ...
    const autoHydrateWorkspaceFromReplyZip = async () => {
      const repliedDoc = (ctx as any).message?.reply_to_message?.document;
      if (!repliedDoc?.file_name?.toLowerCase?.().endsWith(".zip")) return false;
      const file = await bot.telegram.getFile(repliedDoc.file_id);
      if (!file.file_path) throw new Error("Telegram did not return a file path for the replied ZIP.");
      const fileUrl = `https://api.telegram.org/file/bot${config.botToken}/${file.file_path}`;
      const response = await axios.get(fileUrl, { responseType: "arraybuffer" });
      const userId = ctx.from!.id;
      const storageDir = path.join(telegramStorageDir, String(userId));
      fs.mkdirSync(storageDir, { recursive: true });
      const safeName = `${(path.basename(repliedDoc.file_name, ".zip")).replace(/[^a-zA-Z0-9._-]/g, "_")}_${Date.now()}.zip`;
      const zipPath = path.join(storageDir, safeName);
      ...
      state.zips[safeName] = zipPath;
      state.cwd = workspaceDir;
      await ctx.reply(`📦 Replied ZIP imported and extracted.\nWorkspace: \`${path.basename(workspaceDir)}\``, { parse_mode: "Markdown" });
      return true;
    };
    ...
    if (/(run\s+)?git\s+push/i.test(normalizedText) || /(run\s+)?push\s+to\s+github/i.test(normalizedText)) {
      ...
      if (inlineRepo && inlineToken) {
        await autoHydrateWorkspaceFromReplyZip();
        return pushWorkspaceToGithub(ctx, inlineRepo, inlineToken);
      }
      state.pendingGithubPush = true;
      return ctx.reply("Send GitHub repo + token in one message.\nExample:\nrepo: owner/repo\ntoken: ghp_xxx");
    }

    const inlineRepoAny = normalizedText.match(/(?:https?:\/\/github\.com\/[^\s]+|[\w.-]+\/[\w.-]+)/i)?.[0];
    const inlineTokenAny = normalizedText.match(/(gh[pousr]_[A-Za-z0-9_]+|github_pat_[A-Za-z0-9_]+)/i)?.[1];
    if (inlineRepoAny && inlineTokenAny && /push|github|upload|commit/i.test(normalizedText)) {
      await autoHydrateWorkspaceFromReplyZip();
      return pushWorkspaceToGithub(ctx, inlineRepoAny, inlineTokenAny);
    }
  ...
  async function pushWorkspaceToGithub(ctx: Context, repoInput: string, githubToken: string) {
    ...
    const resolveWorkspaceForPush = () => {
      ...
      const workspaceCandidates = fs.existsSync(telegramWorkspacesDir)
        ? fs.readdirSync(telegramWorkspacesDir)
            .filter((entry) => entry.startsWith(userPrefix))
            .map((entry) => ({
              entry,
              fullPath: path.join(telegramWorkspacesDir, entry),
              mtime: fs.statSync(path.join(telegramWorkspacesDir, entry)).mtimeMs
            }))
      ...
      if (workspaceCandidates.length) {
        const fallback = workspaceCandidates[0].fullPath;
        logger.warn(`Invalid workspace path for user ${userId}: "${current}". Falling back to latest workspace: ${fallback}`);
        state.cwd = fallback;
        return fallback;
      }

      throw new Error(`Workspace not found. Current path is invalid: "${current || "(empty)"}". Upload or clone a project first.`);
    };
...
${chunks.join(' | ')}` : '';
  ...
  bot.on("text", async (ctx, next) => {
    ...
    if (state.pendingCreateAppApproval && /^(yes|y)$/i.test(lower)) {
      state.pendingCreateAppApproval = false;
      await createAppFiles(ctx);
      return;
    }
    if (state.pendingCreateAppApproval && /^(no|n)$/i.test(lower)) {
      state.pendingCreateAppApproval = false;
      return ctx.reply("👍 App creation cancelled.");
    }
    ...
    const firstWord = (t.match(/^([^\s]+)/)?.[1] || "").toLowerCase();
    const slashlessCommands = new Set([
    ...
    if (slashlessCommands.has(firstWord)) {
      const tail = t.slice(firstWord.length).trim();
      const normalized = firstWord === "create" ? "build" : firstWord;
  ...
  bot.command("createapp", async (ctx) => {
    const state = getState(ctx.from!.id);
    state.pendingCreateAppApproval = true;
    return ctx.reply(`📂 Proposed app structure:
/src
  index.js
package.json
README.md

Approve app creation? Reply with "yes" or "no".`);
  });

  bot.command("shell", async (ctx) => {
    const cmd = ctx.message.text.replace(/^\/shell(@\w+)?/i, "").trim();
    if (!cmd) return ctx.reply("Usage: /shell <command>");
    return executeShell(ctx, cmd);
  });
...
${users.map((id) => `• ${id}`).join("\n")}` : "No whitelisted users.");
  ...
  bot.command("status", async (ctx) => {
    const health = botStatus === "live" ? "live" : "degraded";
    const qs = AIEngine.queueStatus();
    return ctx.reply(`📊 Bot health: ${health}
Queue: ${qs.pending}/${qs.maxQueueLength} pending (${qs.processing ? "processing" : "idle"})
Last AI used: ${qs.lastProviderUsed}
Active AI providers: ${AIEngine.providerName()}
Whitelisted users: ${allowedUsers.size}`);
  ...
  bot.command("ask", async (ctx) => {
    ...
    const reply = await AIEngine.chat(query, [], "You are the main AI agent handling search and responses.", "auto");
  ...
  bot.command("groq", async (ctx) => {
    const prompt = ctx.message.text.replace(/^\/groq(@\w+)?/i, "").trim();
    if (!prompt) return ctx.reply("Usage: /groq <prompt>");
    const reply = await AIEngine.chat(prompt, [], "You are Groq-mode assistant.", "groq");
    return sendLongTextResponse(ctx, normalizeModelReply(reply));
  });
