...
async function downloadImage(url, retries = 2) {
  const ua = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
  const referrers = ['https://duckduckgo.com/', 'https://www.google.com/', 'https://www.bing.com/', 'https://www.pinterest.com/'];
...
export default {
  ...
  async execute({ sock, message, from, args }) {
    ...
    if (!keyword) {
      return sock.sendMessage(from, { text: '❌ Usage: `.photos <keyword> [count]`\nMax: 10 images' }, { quoted: message });
    }

    const images = await searchDDGImages(keyword, count);
    if (!images.length) return sock.sendMessage(from, { text: '❌ No images found.' }, { quoted: message });

    const downloadResults = await Promise.allSettled(images.map((url) => downloadImage(url)));
    const validBuffers = downloadResults
      .map((r) => (r.status === 'fulfilled' ? r.value : null))
      .filter((buf) => buf && buf.length >= 1024);

    if (!validBuffers.length) return sock.sendMessage(from, { text: '❌ Could not download images.' }, { quoted: message });

    const caption = `📸 *${keyword}* — ${validBuffers.length} images`;
    await Promise.all(validBuffers.map((buf) => sock.sendMessage(from, { image: buf, caption }, { quoted: message })));
    return null;
  }
};
